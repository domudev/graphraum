...
Off-screen nodes stay out of budgeted sets. Manual `setLabels()` still works under `auto` / `focus` until the next view or focus change overwrites the list. Export `selectBudgetedLabelIds`, `selectFocusLabelIds`, and `orderFocusNodeIds` for the same policies outside the overlay.
