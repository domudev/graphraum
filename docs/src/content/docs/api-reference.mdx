...
Supply generic attribute types and `defineVisuals()` to compile domain data into node color, shape, size (or independent width/height), stroke, edge color, width, opacity, style, and markers, and immutable presentation metadata. See [Node & edge presentation](../node-edge-presentation/) for the complete contract.
